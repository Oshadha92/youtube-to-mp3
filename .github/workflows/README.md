YouTube Audio Downloader (GitHub Actions)
=======================================

This repository provides a GitHub Actions workflow that, when run manually, downloads a YouTube video's audio, converts it to a high-quality MP3, and uploads the MP3 as a downloadable workflow artifact.

Files added
- `.github/workflows/download.yml`: workflow_dispatch workflow that accepts a `youtube_url` input.
- `downloader.py`: Python script to download and convert YouTube audio to MP3.
...
1. Create a new GitHub repository and push these files to the `main` branch.
   ...
   git init
   git add .
   git commit -m "Add GitHub Actions YouTube-to-MP3 workflow"
   gh repo create <owner>/<repo> --public --source=. --remote=origin
   git push -u origin main
   ```

2. In the GitHub repository UI, open the `Actions` tab and select the "YouTube to MP3 (workflow_dispatch)" workflow.
3. Click "Run workflow", enter a YouTube URL (required) and start the workflow.
4. When the run completes, open the workflow run and download the artifact named `youtube-mp3`.

Notes & requirements
- The workflow installs `ffmpeg` on the runner (Ubuntu) to perform audio conversion.
- The workflow uses the `requirements.txt` to install `yt-dlp`.
- Artifacts are placed in the `artifacts/` directory by the script and uploaded by the workflow.

Troubleshooting
- If a download fails due to age-restriction or geo-restriction, consider using `yt-dlp` options for cookies or authenticated sessions.
- If the MP3 is not produced, inspect workflow logs to see `yt-dlp` error details.
# Claude Team MCP
